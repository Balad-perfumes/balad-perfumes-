...
export class QuickAddComponent extends Component {
  ...
  handleClick = async (event) => {
    ...
    if (!productGrid) {
      ...
      if (html) {
        const gridElement = html.querySelector('[data-product-grid-content]') || this.#buildPremiumQuickAddGrid(html);
        if (gridElement) {
  ...
  /**
   * Builds a quick-add grid from the premium product template when the standard
   * product grid wrapper is not present.
   * @param {Document} html
   * @returns {Element | null}
   */
  #buildPremiumQuickAddGrid(html) {
    const premiumProduct = html.querySelector('.stagmenfashion-premium-product');
    if (!premiumProduct) return null;

    const titleText = premiumProduct.querySelector('.stagmenfashion-premium-product__title')?.textContent?.trim() || '';
    const productUrl = this.productPageUrl;

    const grid = document.createElement('div');
    grid.className = 'product-information__grid product-information--media-left product-information--quick-add';
    grid.setAttribute('data-product-grid-content', '');
    grid.setAttribute('data-quick-add-premium-fallback', 'true');

    const mediaWrapper = document.createElement('div');
    mediaWrapper.className = 'product-information__media';
    const mainMedia = premiumProduct.querySelector('.stagmenfashion-premium-product__main-media');
    if (mainMedia) {
      mediaWrapper.appendChild(mainMedia.cloneNode(true));
    }
    ...
    groupContent.className = 'group-block-content';

    const title = document.createElement('div');
    title.className = 'view-product-title';

    const eyebrow = document.createElement('p');
    eyebrow.className = 'quick-add-eyebrow';
    eyebrow.textContent = 'Quick View';

    const titleLink = document.createElement('a');
    titleLink.href = productUrl;
    titleLink.textContent = titleText || this.dataset.productTitle || '';
    titleLink.setAttribute('aria-label', titleText || this.dataset.productTitle || 'Product');
    title.appendChild(titleLink);

    const priceLine = premiumProduct.querySelector('.stagmenfashion-premium-product__price-line');
    const productPrice = document.createElement('product-price');
    if (priceLine) {
      productPrice.innerHTML = priceLine.innerHTML;
    }

    const buyButtons = document.createElement('div');
    buyButtons.className = 'buy-buttons-block';

    const productFormComponent = document.createElement('product-form-component');
    const form = premiumProduct.querySelector('.stagmenfashion-premium-product__form');
    if (form) {
      productFormComponent.appendChild(form.cloneNode(true));
    }
    buyButtons.appendChild(productFormComponent);

    groupContent.appendChild(eyebrow);
    groupContent.appendChild(title);
    groupContent.appendChild(productPrice);
    groupContent.appendChild(buyButtons);
  ...
  async updateQuickAddModal(productGrid) {
    ...
    const hasStandardQuickAddContent =
      productGrid.querySelector('.product-details') !== null && productGrid.querySelector('product-form-component') !== null;

    if (isMobileBreakpoint() && hasStandardQuickAddContent && !productGrid.hasAttribute('data-quick-add-premium-fallback')) {
